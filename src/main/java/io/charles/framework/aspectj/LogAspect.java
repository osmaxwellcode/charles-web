...
import io.charles.common.enums.HttpMethod;
...
import io.charles.framework.aspectj.lang.enums.BusinessStatus;
...
public class LogAspect {
    ...
    // 配置织入点
    @Pointcut("@annotation(io.charles.framework.aspectj.lang.annotation.Log)")
    ...
    protected void handleLog(final JoinPoint joinPoint, final Exception e, Object jsonResult) {
        try {
            ...
            // 返回参数
            operLog.setJsonResult(JsonUtil.toJson(jsonResult));
        ...
        if (paramsArray != null && paramsArray.length > 0) {
            for (int i = 0; i < paramsArray.length; i++) {
                if (StringUtils.isNotNull(paramsArray[i]) && !isFilterObject(paramsArray[i])) {
                    String jsonObj = JsonUtil.toJson(paramsArray[i]);
                    params += jsonObj + " ";
                }
            }
        }
